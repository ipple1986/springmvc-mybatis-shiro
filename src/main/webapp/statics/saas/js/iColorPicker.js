...
function iColorShow(id, id2) {
    ...
    jQuery('#colorPreview span').remove();
        ...
        for (j = 0; j < tblChilds.length; j++) {
            ...
            for (k = 0; k < tblCells.length; k++) {
                jQuery(tblChilds[j].childNodes[k]).unbind().mouseover(function (a) {
                    ...
                    if(aaa.length == 4){
                    	araay = aaa.split("");
                    	aaa = "#" + araay[1] + araay[1] + araay[2] + araay[2] + araay[3] + araay[3];
                    }
                    jQuery("#" + id).val(aaa).css("background", aaa);
                    jQuery("#iColorPickerBg").hide();
                    jQuery("#iColorPicker").fadeOut();
                    jQuery("#" + id).prev().prev().css("background", aaa);
                    var $otherInput=$("#" + id);
                    jQuery(this);
                    if($otherInput.attr('id')=='otherInput1000'){
...
this.iColorPicker = function () {
    jQuery("input.iColorPicker").each(function (i) {
        ...
        jQuery(this).css("backgroundColor", jQuery(this).val()).after('<a class="qwe" {width: 50px; position: absolute;} style="width:0px;" href="javascript:void(null)" id="icp_' + this.id + '" onclick="iColorShow(\'' + this.id + '\',\'icp_' + this.id + '\')"><img src="' + imageUrl + '" style="display:none;border:0;margin:0 0 0 3px" align="absmiddle" ></a>')
    })
};
jQuery(function () {
    iColorPicker()
